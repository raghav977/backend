...
class SaleItemSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'total_amount', 'mrp', 'discount_percentage',
            'free_products', 'total_discount', 'net_amount','product_name'
        ]
        read_only_fields = ['total_amount', 'total_discount','product_name', 'net_amount']
    ...
    def validate(self, data):
        ...
        if product.quantity < quantity:
            raise serializers.ValidationError({
                "quantity": f"Insufficient stock for product '{product.name}'. Available quantity is {product.quantity}."
            })
        return data
    ...
    def create(self, validated_data):
        ...
        discount_pct = validated_data.get('discount_percentage', 0) or 0
        total_amount = quantity * mrp
        total_discount = total_amount * (discount_pct / 100)
        net_amount = total_amount - total_discount
...
class SaleSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        for item_data in items_data:
            SaleItemSerializer().create({**item_data, 'sale': sale})
